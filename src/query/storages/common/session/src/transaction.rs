...
impl TxnManager {
    ...
    pub fn get_table_meta_timestamps(
        ...
        if !self.is_active() {
            eprintln!("auto commit mode");
            return TableMetaTimestamps::new(previous_snapshot, delta);
        }

        eprintln!("explicit txn");
        let entry = self.txn_buffer.table_meta_timestamps.entry(table_id);
        match entry {
            Entry::Occupied(e) => {
                eprintln!(
                    "prev snapshot ts {:?}",
                    previous_snapshot.as_ref().map(|s| s.timestamp)
                );
                // let mut ret = TableMetaTimestamps::new(previous_snapshot, delta);
                // let v = *e.get();
                // ret.segment_block_timestamp = v.segment_block_timestamp;
                // ret
                *e.get()
            }
            Entry::Vacant(e) => {
                eprintln!("no cached table meta timestamps");
