...
use crate::io::read::block::block_reader_deserialize::FieldDeserializationContext;
...
pub fn deserialize_field<'a>(
    ...
    column: &ColumnNode,
) -> databend_common_exception<Option<DeserializedArray<'a>>> {
    ...
    if !field_column_metas.is_empty() {
        ...
        let mut array_iter = chunks_to_parquet_array_iter(
            field_column_metas,
            field_column_data,
            num_rows,
...
) -> databend_common_exception::Result<ArrayIter<'a>> {
    let columns = metas
        .iter()
