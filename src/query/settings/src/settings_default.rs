    ...
    pub fn instance() -> Result<Arc<DefaultSettings>> {
        Ok(Arc::clone(DEFAULT_SETTINGS.get_or_try_init(|| -> Result<Arc<DefaultSettings>> {
            ...
            let default_settings = HashMap::from([
                ...
                ("enable_probe_side_bloom_runtime_filter", DefaultSettingValue {
                    value: UserSettingValue::UInt64(0),
                    desc: "Enables probe table statistics to activate adaptive bloom runtime filters for JOIN.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=1)),
                }),
                ("runtime_filter_wait_timeout_ms", DefaultSettingValue {
                    value: UserSettingValue::UInt64(0),
                    desc: "Maximum time in milliseconds that a Fuse scan waits for JOIN runtime filters before reading. Late bloom runtime filters still apply to subsequent Parquet blocks. Default 0 disables blocking wait.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=30_000)),
                }),
                ("join_runtime_filter_selectivity_threshold", DefaultSettingValue {
                    value: UserSettingValue::UInt64(10),
                    desc: "Selectivity threshold (percentage) for bloom join runtime filters. Bloom filters are enabled when build rows are below this percentage of the build table, or of the probe table when probe-side bloom runtime filters are enabled. Default 10 means 10%.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(1..=u64::MAX)),
                }),
                ...
                ("max_broadcast_join_build_rows", DefaultSettingValue {
                    ...
                    desc: "Sets the maximum estimated build-side rows for an automatically selected broadcast join. Setting it to 0 disables the limit. This limit does not apply when enforce_broadcast_join is enabled.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Both,
                    range: Some(SettingRange::Numeric(0..=u64::MAX)),
                }),
                ...
                ("enable_legacy_single_key_hash_join_shuffle", DefaultSettingValue {
                    value: UserSettingValue::UInt64(0),
                    desc: "Uses only the first equi-join key for hash join shuffle. Prefer a query-local SET_VAR hint because this setting may cause data skew. Default 0 uses the selected multi-key distribution.",
                    mode: SettingMode::Both,
                    scope: SettingScope::Session,
                    range: Some(SettingRange::Numeric(0..=1)),
                }),
