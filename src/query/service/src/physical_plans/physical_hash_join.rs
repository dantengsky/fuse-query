...
/// Remove a numeric-to-string round trip from an equality key when comparing it with an integer.
///
...
mod tests {
    ...
    fn removes_safe_integer_string_round_trip_from_join_key() -> Result<()> {
        let left = typed_column(0, DataType::Number(NumberDataType::Int64));
        let right_source = typed_column(1, DataType::Number(NumberDataType::Int32));
        let right = cast_to_string(right_source.clone());

        let (simplified_left, simplified_right) = simplify_integer_string_join_keys(&left, &right)?;

        assert_eq!(simplified_left, &left);
        assert_eq!(simplified_right, &right_source);
        Ok(())
    }
    ...
    fn keeps_potentially_lossy_mixed_sign_string_join_key() -> Result<()> {
        let left = typed_column(0, DataType::Number(NumberDataType::Int64));
        let right_source = typed_column(1, DataType::Number(NumberDataType::UInt64));
        let right = cast_to_string(right_source);

        let (simplified_left, simplified_right) = simplify_integer_string_join_keys(&left, &right)?;

        assert_eq!(simplified_left, &left);
        assert_eq!(simplified_right, &right);
        Ok(())
    }
}
