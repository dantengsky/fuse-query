...
fn unwrap_integer_to_string_cast<'a>(
    ...
    // Upstream asks `classify_conversion(..).is_safe_for_equality_inference()` here, which this
    // branch does not have (`databend_common_expression::conversion` arrives with #19845).
    // Both operands are already known to be integer `Number`s and so is `common_type`, and for
    // that case `classify_conversion` reduces to `Identity` when the types match and otherwise to
    // `classify_number_conversion`, whose only reachable integer/integer arms are
    // `LosslessInjective` exactly when `can_lossless_cast_to` holds and `Lossy` otherwise.
    // `is_safe_for_equality_inference` accepts precisely `Identity | LosslessInjective`, and
    // `can_lossless_cast_to` is reflexive for integers, so it subsumes the `Identity` case too.
